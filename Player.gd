...
func _physics_process(delta):
	...
	# Process view related input (Mouse, right joypad)
...
func process_input(delta):
	...
	if Input.get_connected_joypads().size() > 0:
		...
		# This tutorial assumes you are using a XBOX 360 controller on Linux.
...
func process_view_input(delta):
	...
	# Get the mouse's delta position. This is the amount of movement the mouse has made since last time
	# this function was called. We do this by subtracting the last mouse position from the current mouse position
	var mouse_delta = get_tree().root.get_mouse_position() - last_mouse_pos
	# Update last_mouse_pos, since we will not be needing it again, by setting it to the current mouse position
	last_mouse_pos = get_tree().root.get_mouse_position()
	
	# Rotate the camera holder (everything that needs to rotate on the X-axis) by the relative Y mouse motion.
	rotation_helper.rotate_x(deg2rad(mouse_delta.y * MOUSE_SENSITIVITY))
	...
	self.rotate_y(deg2rad(mouse_delta.x * MOUSE_SENSITIVITY * -1))
	# ----------------------------------
	...
	# We need to clamp the rotation_helper's rotation so we cannot rotate ourselves upside down
	# We need to do this every time we rotate so we cannot rotate upside down with mouse and/or joypad input
	camera_rot = rotation_helper.rotation_degrees
	camera_rot.x = clamp(camera_rot.x, -70, 70)
	rotation_helper.rotation_degrees = camera_rot
